...
interface SimulatorStore extends LeverState {
  ...
  /** Restore all eight levers to SIMULATOR_DEFAULTS. */
  reset: () => void;
...
export const useSimulatorState = create<SimulatorStore>()(
  persist(
    ...
    }),
    {
      ...
      // Only persist the lever fields, not the action functions.
      partialize: (state) => ({
        isc: state.isc,
        ihs: state.ihs,
        cos: state.cos,
...
export type { LeverState, EnglishLevel } from "@/lib/atlas/simulator-fixtures";
